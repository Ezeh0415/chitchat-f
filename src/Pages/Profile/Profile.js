...
const ProfilePage = () => {
  const { userProfile, handleLikedPosts, handleUnLikePosts } = useMyContext();
      ...
      <section>
        ...
        {user?.posts?.length > 0 ? (
          [...user.posts].reverse().map((post) => {
            ...
            const posterEmail = post.email;
            const likerId = user.id;
            const postId = post._id;

            const userHasLiked = post.liked?.some(
              (like) => like.likedByEmail === user.email
            );
            return (
              <main key={post.createdAt}>
                <div className="flex items-center gap-2 mt-6">
                  ...
                  <div className="flex  items-center gap-2">
                    <span className="flex items-center gap-1 capitalize border-r px-1">
                      <h2 className="text-xs"> {post.firstName} </h2>
                      <h2 className="text-xs"> {post.lastName} </h2>
                    </span>
                ...
                <div className="flex items-center gap-5 mt-2">
                  <span className="flex items-center gap-1">
                    ...
                    <p className="text-sm">{post.liked?.length}</p>
                  </span>
          ...
          <div className="flex flex-col items-center justify-center mt-10 animate-fade-in">
            ...
            <svg
              xmlns="http://www.w3.org/2000/svg"
              className="w-32 h-32 text-gray-400 mb-4 animate-bounce"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
              strokeWidth={1.5}
